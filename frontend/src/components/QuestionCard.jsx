...
export default function QuestionCard({ question, onAnswered, onAddToNotebook }) {
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mb-4 whitespace-pre-line text-sm leading-relaxed text-slate-800">
        {question.content}
      </p>
      ...
      <div className="mb-4 space-y-2">
        {Object.entries(question.options).map(([letter, text]) => {
          ...
          let stateClasses = "border-slate-200 hover:border-brand-300";
          if (answered && isCorrect) stateClasses = "border-emerald-400 bg-emerald-50";
          else if (isWrongSelected) stateClasses = "border-rose-400 bg-rose-50";
          else if (isSelected) stateClasses = "border-brand-500 bg-brand-50";
          ...
          return (
            ...
            >
              ...
              {answered && isCorrect && <CheckCircle2 className="h-5 w-5 shrink-0 text-emerald-600" />}
              {isWrongSelected && <XCircle className="h-5 w-5 shrink-0 text-rose-600" />}
            </button>
          );
        })}
      </div>
      ...
      {!answered ? (
        ...
        <div className="space-y-3 border-t border-slate-100 pt-3">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className={result.is_correct ? "font-medium text-emerald-700" : "font-medium text-rose-700"}>
              {result.is_correct ? "Você acertou!" : "Você errou."}
            </span>
            <span className="text-slate-500">XP: {result.xp_earned >= 0 ? "+" : ""}{result.xp_earned}</span>
            <span className="text-slate-500">Sequência: {result.current_streak} dias</span>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-brand-200 px-3 py-1.5 text-xs font-medium text-brand-700 hover:bg-brand-50 disabled:opacity-50"
            >
              {loadingExplanation ? (
            ...
            <span className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-slate-500">
